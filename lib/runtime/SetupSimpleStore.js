import Vue from 'vue'
import { concat, pick, omit, isNil, forEach, isEmpty, merge, set, get, unset, defaultsDeep } from 'lodash'
...
class CreateStore {
  ...
  _decode(value) {
    ...
    if (type === '[object Object]') { return value }
  ...
  _read(key) {
    ...
    if (!isNil(localStorage) && this.options.storages.includes('localStorage')) { merge(result, this._decode(localStorage.getItem(key))) }
    if (!isNil(sessionStorage) && this.options.storages.includes('sessionStorage')) { merge(result, this._decode(sessionStorage.getItem(key))) }
  ...
  _write(key, value) {
    ...
    try {
      if (!isNil(localStorage) && this.options.storages.includes('localStorage')) { localStorage.setItem(key, this._encode(value)) }
      if (!isNil(sessionStorage) && this.options.storages.includes('sessionStorage')) { sessionStorage.setItem(key, this._encode(value)) }
    } catch (e) {
  ...
  _remove(key) {
    ...
    if (!isNil(localStorage) && this.options.storages.includes('localStorage')) { localStorage.removeItem(key) }
    if (!isNil(sessionStorage) && this.options.storages.includes('sessionStorage')) { sessionStorage.removeItem(key) }
  }
  ...
  _sanitize(data) {
    ...
    const { saveProperties, ignoreProperties } = this.options
